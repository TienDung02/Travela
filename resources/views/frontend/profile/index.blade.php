...
@section('content')
    ...
    <!-- Contact Start -->
    <div class="container-fluid contact bg-light py-5 min-h-100vh">
            <div class="p-0 w-60 container  rounded" style="background-color: rgb(242, 242, 242) !important; z-index: 999">
            <div class="position-relative">
                <div class="position-absolute row d-flex bg-white pt-4 rounded w-100 m-auto"  style="transform: translateY(-105%);">
                    ...
                    <div class=" d-flex flex-column justify-content-center col-8">
                        ...
                        @else
                            bg-diamond
                        @endif
                         rounded w-35 p-3 cursor-pointer ms-3 mt-2 text-rank text-white">Member rank {{$user->rank}} <i class="fa-solid fa-medal"></i></h2>
                    </div>
            ...
            <div class="row mt-3 w-100 m-auto">
                <div class="col-lg-4 p-0">
                    <div class="bg-white rounded p-4  me-2">
                        <h4 class="ms-2 mb-4">
                            About You
                        </h4>
                    ...
                    <div class="bg-white rounded p-4 mt-3 me-2">
                        <h4 class="ms-2 mb-4">
...
{{--                                        {{dd($post)}}--}}
                                        <div class="row bg-white rounded w-100 m-auto pb-1 p-3 mb-3">
                                            <div class="col-1">
                                                <img class="avatar-profile-post" src="{{$user->avatar}}" alt="">
                                            </div>
                                            <div class="col-10 p-0 text-start">
                                                <div><b>{{$user->fullname}} </b> in <a href="#" class="text-decoration-underline"><b>{{$post->place->name}}</b></a></div>
                                                <div>{{getDayDifference($post)}}</div>
                                            </div>
                                            <div class="col-1">

                                                <a href="#" class="text-dark fs-3" data-bs-toggle="dropdown"><i class="bi bi-three-dots"></i></a>
                                            ...
                                            <div class="col-12 text-start">
                                                <p class="ms-2 mt-2">{{$post->caption}}</p>
                                            </div>
                                            <div class="grid-container">
                                                @php
                                                    $totalMedia = $post->media->count();
                                                    $visibleMedia = $post->media->take(4);
                                                    $remainingMediaCount = $totalMedia - 4;
                                                @endphp

                                                <div class="media-gallery {{ 'media-count-' . min($totalMedia, 4) }}">
                                                    @foreach($visibleMedia as $index => $media)
                                                        <div class="media-item">
                                                            <img src="{{ $media->media }}" alt="Post Media {{ $index + 1 }}">
                                                ...
                                                <div id="imageModal" class="profile-post-modal">
                                            ...
                                            <div class="row">
                                                <div class="col-4 btn-interact py-2 fs-4 cursor-pointer" id="like-button-{{ $post->id }}" data-post-id="{{ $post->id }}">
                                                    @php
                                                        $userLikedPost = false;
                                                ...
                                                <div class="col-4 py-2 btn-interact fs-4 cursor-pointer border-start border-end">
                                                    <i class="fa-regular fa-comment"></i>
                                                </div>
                                                <div class="col-4 py-2 btn-interact cursor-pointer fs-4">
